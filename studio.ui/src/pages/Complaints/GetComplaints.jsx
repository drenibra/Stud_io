...
export default function GetComplaints() {
  ...
  // get data from db
  useEffect(() => {
    axios.get('https://localhost:7007/GetComplaints').then((response) => {
  ...
  function RefuzoAnkesen(id) {
    const confirmBox = window.confirm('A jeni te sigurte qe deshironi te refuzoni ankesen me id ' + id + '?  ');
    if (confirmBox === true) {
      axios.delete('https://localhost:7007/DeleteComplaint/' + id).then(() => {
          ...
          {complaints.map((complaint) => {
            ...
            return (
              <Card key={id} sx={{ maxWidth: 345 }} className="card-container-ankesat">
                ...
                <br /> <br /> <br />
                <CardActions>
                  <Button size="small" onClick={() => RefuzoAnkesen(id)} className="btn-card" variant="contained">
                    Delete
                  </Button>
                </CardActions>
              </Card>
